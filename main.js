...
let title = `<h1>ガチで東京の人しか解けない！ #東京の難読地名クイズ</h1>`
document.getElementById('main').insertAdjacentHTML('beforebegin', title);
...
for (let i = 0; i < 10; i++) {
  let content =
    ...
    `<div id='result_box${i}' class='invisible'>` +
    `<p id='true${i}' class='invisible'>正解！</p>` +
    `<p id='false${i}' class='invisible'>不正解！</p>` +
    `<p id='description${i}' class='invisible'>正解は${selections[i][0]}です！</p>` +
    `</div>` +
    `</div>`;
  document.getElementById('main').insertAdjacentHTML('beforeend', content);

  //選択肢の部分  シャッフルのためにcreateElementを用いて書いていきます
  ...
  let get_id_result_box = document.getElementById('result_box' + [i]);
  let get_id_true = document.getElementById('true' + [i]);
  let get_id_false = document.getElementById('false' + [i]);
  let get_id_description = document.getElementById('description' + [i]);
  ...
  get_id_choice0.onclick = function () {
    ...
    get_id_result_box.className = 'result_box';
